...
__global__ void sobel(unsigned char *s, unsigned char *t, unsigned height, unsigned width, unsigned channels) {
    ...
    int x = tid, now = yBound;
    for (int v = -yBound; v <= yBound; ++v) {
        for (int u = -xBound; u <= xBound; ++u) {
            if(bound_check(x + u, 0, width) && bound_check(v, 0, height)){
                sB[v+yBound][u+threadIdx.x+xBound] = s[channels*(width*(0+v)+(x+u))+0];
                sG[v+yBound][u+threadIdx.x+xBound] = s[channels*(width*(0+v)+(x+u))+1];
                sR[v+yBound][u+threadIdx.x+xBound] = s[channels*(width*(0+v)+(x+u))+2];
            }
        }
    }
    __syncthreads();
    for (int y = 0; y < height; ++y) {
        ...
        for (int i = 0; i < Z; ++i) {
            ...
            for (int v = -yBound; v <= yBound; ++v) {
                for (int u = -xBound; u <= xBound; ++u) {
                    const unsigned char R = sR[(v+now+Y)%Y][u+threadIdx.x+xBound];
                    const unsigned char G = sG[(v+now+Y)%Y][u+threadIdx.x+xBound];
                    const unsigned char B = sB[(v+now+Y)%Y][u+threadIdx.x+xBound];
        ...
        for (int u = -xBound; u <= xBound; ++u) {
            if(bound_check(x + u, 0, width) && bound_check(y+1+yBound, 0, height)){
                sR[(-yBound+now+Y)%Y][u+threadIdx.x+xBound] = s[channels*(width*(y+1+yBound)+(x+u))+2];
                sG[(-yBound+now+Y)%Y][u+threadIdx.x+xBound] = s[channels*(width*(y+1+yBound)+(x+u))+1];
                sB[(-yBound+now+Y)%Y][u+threadIdx.x+xBound] = s[channels*(width*(y+1+yBound)+(x+u))+0];
            }
        }
        __syncthreads();
        now = (now+1)%Y;
...
int main(int argc, char **argv) {
    ...
    // launch cuda kernel
    // sobel<<<num_blocks, num_threads, 5*sharedWidth*channels*sizeof(unsigned char)>>>(dsrc, ddst, height, width, channels);
