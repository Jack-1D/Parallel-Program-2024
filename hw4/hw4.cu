...
int main(int argc, char *argv[]) {
    ...
    cudaMemcpy2DAsync(d_O, pitchO, O, N * d * sizeof(float), N * d * sizeof(float), B, cudaMemcpyHostToDevice, stream[0]);
    ...
    cudaMemcpy2DAsync(O, N * d * sizeof(float), d_O, pitchO, N * d * sizeof(float), B, cudaMemcpyDeviceToHost, stream[0]);
