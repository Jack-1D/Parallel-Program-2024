...
__global__ void flash_attention_kernel(float* d_kj, float* d_vj, float *q, float *o, float* d_l, float* d_m, int br, int bc, int d, int tr, float scalar){
    ...
    __shared__ float qi[TileSize * 64];
    __shared__ float kj[TileSize * 64];
    __shared__ float vj[TileSize * 64];
    __shared__ float mi[TileSize];
    __shared__ float li[TileSize];
    __shared__ float oi[TileSize * 64];

    if(bcIdx == 0){
        for(int t = 0;t < d;++t){
            qi[brIdx * d + t] = q[globalBrIdx * d + t];
        }
    }
    if(brIdx == 0){
        for(int t = 0;t < d;++t){
            kj[bcIdx * d + t] = d_kj[bcIdx * d + t];
        }
    }
    if(bcIdx == 0){
        ...
        for(int t = 0;t < d;++t){
            oi[brIdx * d + t] = o[globalBrIdx * d + t];                
        }
    }
    if(brIdx == 0){
        for(int t = 0;t < d;++t){
            vj[bcIdx * d + t] = d_vj[bcIdx * d + t];
        }
    }
    __syncthreads();
    ...
    // Query Loop
        ...
        if(brIdx < br && bcIdx < bc){
            shared_sij[brIdx * bc + bcIdx] = 0.0F;
            for (int t = 0; t < d; t++) {
                shared_sij[brIdx * bc + bcIdx] += qi[brIdx * d + t] * kj[bcIdx * d + t];
            }
            shared_sij[brIdx * bc + bcIdx] *= scalar;
        }
        __syncthreads();
        // RowMax
        if(bcIdx == 0 && brIdx < br){
            shared_mij[brIdx] = shared_sij[brIdx * bc];
            for (int j = 0; j < bc; j++) {
                shared_mij[brIdx] = fmax(shared_mij[brIdx], shared_sij[brIdx * bc + j]);
            }
        }
        __syncthreads();
        // MinusMaxAndExp
        if(bcIdx < bc && brIdx < br){
            shared_pij[brIdx * bc + bcIdx] = expf(shared_sij[brIdx * bc + bcIdx] - shared_mij[brIdx]);
        }
        __syncthreads();
        // RowSum
        if(bcIdx == 0 && brIdx < br){
            ...
            for (int j = 0; j < bc; j++) {
                shared_lij[brIdx] += shared_pij[brIdx * bc + j];
            }
        }
        __syncthreads();        
        ...
        if(bcIdx == 0 && brIdx < br){
            ...
            for (int j = 0; j < d; j++) {
                float pv = 0.0F;
                for (int t = 0; t < bc; t++) {
                    pv += shared_pij[brIdx * bc + t] * vj[t * d + j];
                }
                o[globalBrIdx * d + j] = (li[brIdx] * expf(mi[brIdx] - mi_new) * oi[brIdx * d + j] + expf(shared_mij[brIdx] - mi_new) * pv) / li_new;
            }
